...
fn test_link_show_vlan() {
    with_netns(|ns| {
        ...
        with_vlan_iface(ns, vlan_name, &[], || {
            let expected_output =
                ns.exec_cmd(&["ip", "link", "show", vlan_name]);

            let our_output = ns.ip_rs_exec_cmd(&["link", "show", vlan_name]);
...
fn test_link_detailed_show_vlan() {
    with_netns(|ns| {
        ...
        with_vlan_iface(ns, vlan_name, &[], || {
            let expected_output =
                ns.exec_cmd(&["ip", "-d", "link", "show", vlan_name]);

            let our_output =
                ns.ip_rs_exec_cmd(&["-d", "link", "show", vlan_name]);
...
fn test_vlan_protocol() {
    with_netns(|ns| {
        ...
        with_vlan_iface(ns, vlan_name, &["protocol", "802.1ad"], || {
            let expected_output =
                ns.exec_cmd(&["ip", "-d", "link", "show", vlan_name]);
...
fn test_vlan_reorder_hdr_on() {
    with_netns(|ns| {
        ...
        with_vlan_iface(ns, vlan_name, &["reorder_hdr", "on"], || {
            let expected_output =
                ns.exec_cmd(&["ip", "-d", "link", "show", vlan_name]);
...
fn test_vlan_reorder_hdr_off() {
    with_netns(|ns| {
        ...
        with_vlan_iface(ns, vlan_name, &["reorder_hdr", "off"], || {
            let expected_output =
                ns.exec_cmd(&["ip", "-d", "link", "show", vlan_name]);
...
fn test_vlan_gvrp_on() {
    with_netns(|ns| {
        ...
        with_vlan_iface(ns, vlan_name, &["gvrp", "on"], || {
            let expected_output =
                ns.exec_cmd(&["ip", "-d", "link", "show", vlan_name]);
...
fn test_vlan_gvrp_off() {
    with_netns(|ns| {
        ...
        with_vlan_iface(ns, vlan_name, &["gvrp", "off"], || {
            let expected_output =
                ns.exec_cmd(&["ip", "-d", "link", "show", vlan_name]);
...
fn test_vlan_mvrp_on() {
    with_netns(|ns| {
        ...
        with_vlan_iface(ns, vlan_name, &["mvrp", "on"], || {
            let expected_output =
                ns.exec_cmd(&["ip", "-d", "link", "show", vlan_name]);
...
fn test_vlan_mvrp_off() {
    with_netns(|ns| {
        ...
        with_vlan_iface(ns, vlan_name, &["mvrp", "off"], || {
            let expected_output =
                ns.exec_cmd(&["ip", "-d", "link", "show", vlan_name]);
...
fn test_vlan_loose_binding_on() {
    with_netns(|ns| {
        ...
        with_vlan_iface(ns, vlan_name, &["loose_binding", "on"], || {
            let expected_output =
                ns.exec_cmd(&["ip", "-d", "link", "show", vlan_name]);
...
fn test_vlan_loose_binding_off() {
    with_netns(|ns| {
        ...
        with_vlan_iface(ns, vlan_name, &["loose_binding", "off"], || {
            let expected_output =
                ns.exec_cmd(&["ip", "-d", "link", "show", vlan_name]);
...
fn test_vlan_bridge_binding_on() {
    with_netns(|ns| {
        ...
        with_vlan_iface(ns, vlan_name, &["bridge_binding", "on"], || {
            let expected_output =
                ns.exec_cmd(&["ip", "-d", "link", "show", vlan_name]);
...
fn test_vlan_bridge_binding_off() {
    with_netns(|ns| {
        ...
        with_vlan_iface(ns, vlan_name, &["bridge_binding", "off"], || {
            let expected_output =
                ns.exec_cmd(&["ip", "-d", "link", "show", vlan_name]);
...
fn with_vlan_iface<T>(ns: &NetnsGuard, vlan_name: &str, opts: &[&str], test: T)
where
    T: FnOnce(),
{
